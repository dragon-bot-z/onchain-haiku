...
contract OnchainHaiku {
    ...
    function mint() external payable returns (uint256) {
        ...
        _seeds[tokenId] = uint256(keccak256(abi.encodePacked(block.prevrandao, msg.sender, tokenId, block.timestamp)));
    ...
    function getHaiku(uint256 tokenId)
        public
        view
    ...
    function _generateSVG(string memory l1, string memory l2, string memory l3, uint256 tokenId)
        internal
        pure
        returns (string memory)
    {
        ...
        return string(
            abi.encodePacked(
                ...
                _toString((hue + 30) % 360),
                ',20%,12%)"/>',
                "</linearGradient></defs>",
                '<rect width="400" height="500" fill="url(#bg)"/>',
                '<text x="200" y="160" text-anchor="middle" fill="hsl(',
                _toString(hue),
                ',40%,75%)" ',
                'font-family="Georgia,serif" font-size="18" font-style="italic">',
                l1,
                "</text>",
                '<text x="200" y="240" text-anchor="middle" fill="hsl(',
                _toString((hue + 120) % 360),
                ',40%,75%)" ',
                'font-family="Georgia,serif" font-size="18" font-style="italic">',
                l2,
                "</text>",
                '<text x="200" y="320" text-anchor="middle" fill="hsl(',
                _toString((hue + 240) % 360),
                ',40%,75%)" ',
                'font-family="Georgia,serif" font-size="18" font-style="italic">',
                l3,
                "</text>",
                '<text x="200" y="460" text-anchor="middle" fill="hsl(',
                _toString(hue),
                ',20%,40%)" ',
    ...
    function _generateJSON(string memory l1, string memory l2, string memory l3, uint256 tokenId, string memory svg)
        internal
        pure
        returns (string memory)
    {
        return string(
            abi.encodePacked(
                ...
                '",',
                '"attributes":[',
                '{"trait_type":"Line 1","value":"',
                l1,
                '"},',
                '{"trait_type":"Line 2","value":"',
                l2,
                '"},',
                '{"trait_type":"Line 3","value":"',
                l3,
                '"}],',
